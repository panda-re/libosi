...
public:
    kernel(std::shared_ptr<VirtualMemory> vm, vm_addr_t kdg,
           struct StructureTypeLibrary* lib)
    {
        vmem = vm;
        kdbg = kdg;
        tlib = lib;
    ...
    std::set<uint64_t> get_process_list()
    {
        ...
        if (ptr_width == 4) {
            eproc_address = get_address_active_process_head(vmem.get(), kdbg) -
        ...
        do {
            ...
            // _EPROCESS.is_valid from volatility, used to filter out PsActiveProcessHead
            // header
            if (dtb == 0 || (dtb % 0x20) != 0) {
    ...
    {
        for (std::set<uint64_t>::iterator it = proc_list.begin(); it != proc_list.end();
             ++it) {
