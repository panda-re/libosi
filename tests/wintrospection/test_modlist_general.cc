...
void handle_proces_modlist(struct WindowsKernelOSI* wintro, struct process* p, std::map<uint64_t, std::vector<struct ModuleInfo>> EXPECTED_RESULTS)
{
    ...
    if (modlist) {
        ...
        ASSERT_TRUE(entry.size() == 0) << "Didn't find a module list where one was expected for PID: " << pid;
    }
...
TEST_P(TestMlist, snapshot)
{
    ...
    for (int i =0; i < snapshots.size(); i++)
    {
        ...
        //fprintf(stderr, "g_profile: %s\t asid: %lu\t kdbg: %lu\t kpcr: %lu\t pointer_width: %i\n", g_profile.c_str(), g_asid, g_kdbg, g_kpcr, g_pointer_width);
        ...
        ASSERT_TRUE(numProc >= EXPECTED_RESULTS.size()) << "Mismatched number of processes, expecting " << EXPECTED_RESULTS.size() << " found " << numProc;
...
INSTANTIATE_TEST_CASE_P(Default, TestMlist,
  testing::ValuesIn(
