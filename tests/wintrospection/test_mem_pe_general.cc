...
TEST_P(TestMemPElist, snapshot)
{
    ...
    for (int i =0; i < snapshots.size(); i++)
    {
        ...
        std::string dlllist_path = std::string(DLLLISTDIR + snapshot_name + "_dlllist.json");
        ...
        ASSERT_TRUE(access(dlllist_path.c_str(), R_OK) == 0) << "Could not read input dlllist file";
        ...
        for (auto const &x: EXPECTED_RESULTS) {
            ProcessOSI* posi = (ProcessOSI*) std::malloc(sizeof(ProcessOSI));
            std::memset(posi, 0, sizeof(ProcessOSI));
            ASSERT_TRUE(init_process_osi_from_pid(&kosi, posi, x.first)) << "Failed to initialize process OSI object";
            ...
            for (auto const &mod: x.second) {
                ...
                ASSERT_TRUE(TRANSLATE_SUCCEEDED(posi->vmem->read(mod.base + nt_header_address, &pe_sign, 4))) << "Failed to read pe signature for base "
                << mod.base << " and NT address " << nt_header_address << " for pid " << x.first << "with asid " << posi->vmem->get_asid();
        ...
        ASSERT_EQ(numPEFound, numPESuccess) << "Number of PEs identified and parsed not equal";
    }
    delete_snapshot_dir();
}
...
INSTANTIATE_TEST_CASE_P(Default, TestMemPElist,
  testing::ValuesIn(
