...
TEST(TestAmd64CallTracer, Win7SP1Amd64)
{
    ...
    ASSERT_TRUE(initialize_windows_kernel_osi(&s_kosi, &kdetails, asid, pae)) << "Failed to initialize kernel osi";
    ...
    osi::i_t eproc = osi::i_t(posi.vmem, kosi->kernel_tlib, posi.eprocess_address, "_EPROCESS");
    ...
    ASSERT_TRUE(LDR_DATA_ENTRY.get_address() == 0x002E2010 ) << "wrong ldr list entry addr";
