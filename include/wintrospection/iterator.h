    ...
    class iterator {
        ...
        public:
            iterator(i_t &obj, const std::string &m) {
                object = obj;
                member = m;
                struct MemberResult* mem = offset_of(obj.get_type_library(),
                              obj.get_type(), m.c_str());
    ...
    class iterator32 {
        ...
        public:
            ...
            iterator32 &prev() {
                ...
                i_t blink = i_t(object.get_virtual_memory_shared(), object.get_type_library(), blink_address, "LIST_ENTRY32");
                object.set_address(blink.get_address() - tid_offset);
                return *this;
            }
            ...
            bool has_next() {
                if (object[member].get_address() != 0) {
                    ...
                    auto flink = i_t(object.get_virtual_memory_shared(), object.get_type_library(), flink_address, "LIST_ENTRY32");
                    try {
                        // TODO use virtual memory handle to check if valid
                        flink.getu();
                        return true;
            ...
            bool operator==(iterator32 &that) const {
                return object == that.get();
            }
