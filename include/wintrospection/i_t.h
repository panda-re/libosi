...
namespace osi {
    class i_t {
        ...
        public:
            ...
            struct StructureTypeLibrary* get_type_library() {
                return m_tlib;
            }

            const struct StructureType* get_type() const {
                return m_type;
            }

            VirtualMemory* get_virtual_memory()
            {
                return m_vmem.get();
            }

            std::shared_ptr<VirtualMemory> get_virtual_memory_shared()
            {
                return m_vmem;
            }


            vm_addr_t get_address() const {
                return m_address;
            }
            ...
            bool is_pointer() const {
                return is_pointer_structure_type(m_type);
            }
            ...
            i_t dereference() const
            {
                ...
                if (status != TSTAT_SUCCESS) {
                    throw std::runtime_error(
                        "invalid dereference: vmem->read failed");
                }
                return i_t(m_vmem, m_tlib, new_addr, dereference_st(m_tlib, m_type));
            }

            i_t operator[](const std::string &m) const {
                return get_member(m);
            }

            i_t operator()(const std::string &m) const {
                return get_member(m).dereference();
            }
            ...
            vm_addr_t operator&() const {
                return get_address();
            }

            bool operator==(const i_t &that) const {
                return get_address() == that.get_address() && equal_structure_types(get_type(), that.get_type());
            }
